...
- [~] **Blur Gallery** (M): **Tilt-Shift** **done** — a graduated/positional blur that keeps a horizontal focus band sharp and blurs progressively outside it (focus center / band half-width / feather / max blur / tilt angle); destructive on the active layer, undoable (region-COW), wired through the filter shader (`filter.wgsl` kind 30, per-pixel focus weight × local 2D Gaussian) → compositor `apply_tilt_shift` → Filter ▸ Blur menu. CPU reference of the focus-weight falloff + full tilt-shift (3 unit tests) + 1 GPU pixel test (skip-on-no-adapter). *Still:* Field, Iris, Path, Spin blur; bokeh shape; Lens Blur (depth/alpha-aware).
