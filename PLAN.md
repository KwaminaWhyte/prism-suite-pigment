...
- [~] **Workspace & panels** (M): **done** — scrollable + collapsible panels, **tool groups + flyouts** (23 → 12 family buttons), and a **contextual tool-options bar** across the top (per-tool size/hardness/opacity, Liquify/Detail modes, Fill/selection options moved out of the right panel; right panel is now layer/document properties only). *Still:* dockable/floating panels, **save/load + shareable workspaces** (Affinity Studio-preset model), Window-menu panel show/hide, reset. See [../UI_UX.md](../UI_UX.md).
