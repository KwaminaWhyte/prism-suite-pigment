...
- [~] **Layer styles / FX** (L): **Stroke done** — non-destructive outer stroke evaluated live in the composite shader (alpha-edge ring, tinted, drawn behind the layer); per-layer color + width, GPU pixel-tested. *Still:* Drop Shadow, Inner Shadow, Outer/Inner Glow, Bevel & Emboss, Color/Gradient/Pattern Overlay, Satin; per-effect blend/opacity; copy/paste/scale styles. (FX-pass machinery now proven — shadow/glow reuse the same alpha-neighborhood sampling.)
...
- [~] **Workspace & panels** (M): **done** — scrollable + collapsible panels (left palette + right properties scroll; FX/Blend-If/Channels/Histogram collapse) and **tool groups + flyouts** (23 tools → 12 family buttons; multi-tool groups open a flyout, active variant's icon shown). *Still:* **contextual tool-options bar** across the top (move per-tool controls out of the right panel), dockable/floating panels, **save/load + shareable workspaces** (Affinity Studio-preset model), Window-menu panel show/hide, reset. See [../UI_UX.md](../UI_UX.md).
