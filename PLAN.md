...
- [~] **Workspace & panels** (M): **scrollable + collapsible panels done** (left tool palette + right properties both scroll; FX/Blend-If/Channels/Histogram sections collapse). *Still:* tool **groups + flyouts** (23 flat tools is too many — group by family like Affinity), **contextual tool-options bar** across the top (move per-tool controls out of the right panel), dockable/floating panels, **save/load + shareable workspaces** (Affinity Studio-preset model), Window-menu panel show/hide, reset. See [../UI_UX.md](../UI_UX.md).
