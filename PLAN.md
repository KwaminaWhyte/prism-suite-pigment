...
- [~] **Layer styles / FX** (L): **Stroke + Drop Shadow done** — non-destructive, evaluated live in the composite shader: outer stroke (alpha-edge ring) + drop shadow (blurred, offset, tinted alpha drawn behind); per-layer color/width and color/offset/blur, GPU pixel-tested. *Still:* Inner Shadow, Outer/Inner Glow (glow ≈ shadow with no offset — reuses the same sampler), Bevel & Emboss, Color/Gradient/Pattern Overlay, Satin; per-effect blend/opacity; copy/paste/scale styles.
